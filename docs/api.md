...
Task sync writes `.locks/pipeline/tasks.lock.json`. The lock records the generator version, config path, prefix, runners, targets, resolved manifest paths, generated command names and values, and a rendered hash. Existing unmanaged scripts or Deno tasks are never overwritten; conflicts throw `ASYNC_PIPELINE_SYNC_CONFLICT`. Checks still accept the legacy `.async-pipeline/tasks.lock.json` path while repos migrate.
...
`github generate` writes `.github/workflows/async-pipeline.yml` and `.locks/pipeline/github-workflow.lock.json` unless paths are overridden. Checks still accept the legacy `.github/async-pipeline.lock.json` path while repos migrate.
...
Generated GitHub jobs that contain agent steps package prompt files, redacted transcripts, context packs, and explicit agent task outputs through `async/actions/agent-evidence`; comments use metadata-only summaries rather than pasting patches, logs, or transcripts inline.
